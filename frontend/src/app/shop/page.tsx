    ...
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-6">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        ...
        <nav aria-label="مرتب‌سازی" className="flex gap-2 text-sm">
          {["newest", "price_asc", "price_desc"].map((item) => (
            <Link
              key={item}
              href={href(item)}
              className={`rounded-full px-4 py-2 ${sort === item ? "bg-lajvard text-white" : "bg-char/8 dark:bg-white/10"}`}
            >
              {item === "newest" ? "جدیدترین" : item === "price_asc" ? "ارزان‌ترین" : "گران‌ترین"}
            </Link>
          ))}
        </nav>
      </div>
      <form className="mb-8 grid gap-3 rounded-wobble bg-surface p-4 shadow-shelf sm:grid-cols-2 lg:grid-cols-6" aria-label="فیلتر محصولات">
        ...
        {brands.length > 0 && (
          <select name="brand" defaultValue={searchParams.brand ?? ""} className="rounded-xl border border-char/15 bg-transparent px-3 py-2 text-sm dark:border-white/15">
        ...
        <label className="flex items-center gap-2 px-2 text-sm">
          <input type="checkbox" name="in_stock_only" value="true" defaultChecked={searchParams.in_stock_only === "true"} /> فقط موجودها
        </label>
        <button className="glaze-edge bg-lajvard px-4 py-2 text-sm font-medium text-white" type="submit">
          اعمال فیلتر
        </button>
      </form>
      {!data || !data.items.length ? (
        <EmptyState title="محصولی پیدا نشد" description="به‌زودی قطعه‌های تازه‌ای از کوره اضافه می‌کنیم. فیلترها را تغییر دهید." />
      ) : (
        <>
              ...
              {Array.from({ length: data.pages }, (_, i) => i + 1).map((n) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex h-11 w-11 items-center justify-center rounded-xl ${n === page ? "bg-lajvard text-white" : "bg-char/8 dark:bg-white/10"}`}
                  >
