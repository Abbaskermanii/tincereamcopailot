...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${article.title} | مجله آنیمور سرام`;
  const description =
    article.excerpt?.slice(0, 160) ?? `مقاله «${article.title}» در مجله آنیمور سرام`;
...
export default async function ArticlePage({ params }: Props) {
  ...
  const related = (allArticles ?? [])
    .filter((a) => a.slug !== article.slug && a.category_name === article.category_name)
    .slice(0, 3);
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-6">
      ...
      <article className="grid gap-10 lg:grid-cols-[1fr_320px]">
        ...
        <div className="min-w-0">
          ...
          {/* Cover */}
          {article.cover_url && (
            <figure className="mt-8 overflow-hidden rounded-wobble-card bg-surface shadow-lifted dark:bg-[#1c1a18]">
          ...
          {article.category_name && (
            <div className="mt-10 flex items-center gap-2 border-t border-char/10 pt-6 dark:border-white/10">
      ...
      {related.length > 0 && (
        <section className="mt-16" aria-labelledby="related-articles">
          <h2 id="related-articles" className="mb-6 text-2xl font-extrabold">
            مقالات مرتبط
          </h2>
